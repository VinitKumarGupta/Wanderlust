...
main()
    .then(() => {
        console.log("Connected to Database: " + (MONGO_URL.includes("mongodb+srv") ? "MongoDB Atlas" : "Local MongoDB"));
    })
    .catch((err) => {
        console.log(err);
    });
...
const initDB = async () => {
    ...
    dummyData.data = dummyData.data.map((obj) => ({
        ...obj,
        owner: "69c72f8d250b92b3ed713225", // VinitKumarGupta as the owner of all current listings.
        // Schema logically requires 'geometry' now, adding a default coordinate point logic prevents validation failing.
        geometry: {
            type: "Point",
            coordinates: [77.2090, 28.6139], // Defaulting to somewhere (e.g. New Delhi)
        },
